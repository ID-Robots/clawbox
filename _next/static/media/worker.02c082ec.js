...
let shellPlaceholderTransformer;
...
const ready = (async () => {
  const [
    ...
    { shellPlaceholderTransformer: placeholderTransformer },
  ] = await Promise.all([
    import('./bundle.ts'),
    import('shiki/core'),
    ...
    import('@mintlify/mdx/plugins/shell-placeholder-transformer'),
  ]);
...
function highlightSync(opts) {
  ...
  if (lang) {
    ...
    html = highlighter[serializer](code, {
      ...
      transformers:
        shikiLang === 'bash' ? [...transformers, shellPlaceholderTransformer] : transformers,
      ...opts.opts,
    });
  }
