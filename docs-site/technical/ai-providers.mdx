...
| Level | Behavior |
|---|---|
| `off` | No extended reasoning — the default. Keeps replies fast; you opt in to reasoning when you need it. |
| `high` | Extended reasoning for harder tasks. |
| `xhigh` | Maximum reasoning. |

**The set of levels is per model, not global.** Each provider and model advertises its own supported levels, and the chat header only offers what the selected model actually accepts. Some models expose a different set, some expose more, and models with no reasoning control at all — such as the on-device local model — hide the dial entirely. Selecting a level a model does not support is clamped down to the nearest one it does. So the three above apply to ClawBox AI Flash and Pro, not to every model you can run on a ClawBox.

<Warning>
**The available levels are set programmatically, not by hand-editing config.** Editing `openclaw.json` by hand does not stick — two separate mechanisms will overwrite it:

1. **The gateway re-applies its expected values on every start.** ClawBox's boot-time configuration step rewrites the reasoning-effort metadata on the ClawBox AI models each time the gateway starts, so a manual edit survives only until the next restart.
...
If the levels are not declared, the gateway rejects the higher effort settings and the chat header's **X-High** option silently fails. The levels must therefore be changed in ClawBox's setup and boot-time configuration code, which is the single source of truth.
</Warning>
