...
// Re-enabled (#114): the GH-Actions timeouts were Turbopack recompiling
// routes under load on the single shared `bun run dev` server. The e2e
// webServer now serves a production build (playwright.config.ts) — no
// per-request compile — so this passes on the runner.
