...
/** One table for the whole desktop: the workspace tree draws up to 500 rows
 *  from it and the Files app one per entry, so it is built once. */
...
const DEFAULT_ICON = { icon: "draft", color: "#6b7280" };
const FOLDER_ICON = { icon: "folder", color: "#f97316" };

export function fileIcon(name: string, type: "file" | "directory"): { icon: string; color: string } {
  if (type === "directory") return FOLDER_ICON;
  return EXTENSION_ICONS[fileExtension(name)] ?? DEFAULT_ICON;
}
