    ...
    return (
      <div className="h-full flex flex-col items-center justify-center gap-6 p-8 overflow-y-auto" style={{ background: "#1e1e1e" }}>
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-xl font-semibold text-white">{t("vscode.title")}</h2>
        </div>

        <div className="max-w-md text-center space-y-3">
          <p className="text-white/60 text-sm">
            {t("vscode.notRunning", { port })}
          </p>
          <p className="text-white/40 text-xs">
            {t("vscode.installInstructions")}
          </p>
        </div>

        <div className="w-full max-w-lg space-y-2">
          <p className="text-xs text-white/50 font-medium">{t("vscode.installLabel")}</p>
...
{`curl -fsSL https://code-server.dev/install.sh | sh`}
          ...
          <p className="text-xs text-white/50 font-medium mt-3">{t("vscode.configureLabel")}</p>
          <pre className="bg-black/40 rounded-lg p-3 text-xs text-green-400 font-mono overflow-x-auto border border-white/10">
...
cert: false`}
          ...
          <p className="text-xs text-white/50 font-medium mt-3">{t("vscode.startLabel")}</p>
          <pre className="bg-black/40 rounded-lg p-3 text-xs text-green-400 font-mono overflow-x-auto border border-white/10">
{`code-server --bind-addr 0.0.0.0:${port} --auth none`}
          </pre>

          <p className="text-xs text-white/50 font-medium mt-3">{t("vscode.serviceLabel")}</p>
          <pre className="bg-black/40 rounded-lg p-3 text-xs text-green-400 font-mono overflow-x-auto border border-white/10">
{`sudo systemctl enable --now code-server@$USER`}
  ...
  // Load code-server via same-origin proxy to avoid port/CSP/CORS issues
  const baseUrl = "/code-server";
  let src: string;
  if (filePath) {
