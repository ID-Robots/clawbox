...
interface ChatPopupProps {
  isOpen: boolean
  onClose: () => void
  mascotX?: number // vw position of mascot for anchoring
}
  ...
  if (Array.isArray(m.content)) {
    return m.content
      .map((block: unknown) => {
        ...
        if (b.type === 'thinking') return '' // skip thinking blocks
        return ''
      })
      .filter(Boolean)
      .join('\n')
...
function ChatPopup({ isOpen, onClose, mascotX }: ChatPopupProps) {
  ...
  return (
    ...
    >
      ...
      <div
        ...
        <img src="/clawbox-crab.png" alt="" style={{ width: 28, height: 28, objectFit: 'contain' }} />
        <span style={{ color: '#fff', fontWeight: 600, fontSize: 14, flex: 1 }}>ClawBox Chat</span>
        {status === 'connecting' && (
          <div style={{
            width: 12, height: 12,
