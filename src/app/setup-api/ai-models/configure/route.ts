...
async function configureClawboxAi(
  ...
  // Did this pass actually CHANGE the credential? Only a token SUPPLIED to this
  // call can have: with none, `getConfiguredClawboxAiToken` returned the one
  // already in the store, so the paste below re-writes the same bytes. The
  // question matters because this function runs on paths that are not about
  // ClawBox AI at all — `ensureFallbackModel` calls it with no token whenever a
  // box with no local model saves ANY provider — and everything below keyed off
  // "the credential is new" would otherwise fire on those too.
  const suppliedToken = preferredToken?.trim() ?? "";
  const credentialChanged = suppliedToken !== "" && suppliedToken !== (await storedClawboxAiToken());
