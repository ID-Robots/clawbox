...
export async function POST(request: Request) {
  ...
  try {
    ...
    console.error("[coding-agent] projects/desktop:", err instanceof Error ? err.message : err);
    return NextResponse.json({ error: "Adding the app to the desktop failed.", kind: "failed" }, { status: 500 });
  }
}
