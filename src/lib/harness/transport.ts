...
export interface HarnessAdapter {
   ...
   * commands to offer. A Hermes box whose dashboard is down is the real case,
   * and its route reports it as `available: false` with an empty list; the
   * composer's reaction to that and to a rejection is the same (no popover),
   * which is why the route does not turn it into an error the browser console
   * would carry over a chat that works.
   *
   ...
   * when to ask again — on every reconnect, because a box that installs a skill
   * gains a command.
   */
