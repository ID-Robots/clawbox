...
export interface ResolvedEmbedder {
  ...
  requestUrl: string;
  /**
   * How wide a vector it answers with.
   *
   * CARRIED, not written in a comment, because the index's memory budget is a
   * function of it: a chunk costs `dimensions × 4` bytes of float32, and the
   * cloud model is three times as wide as the one on the box. The ceiling and
   * the vector cache are derived from this (`maxIndexChunks`), so the budget
   * they were sized against holds on either arm instead of tripling under one
   * of them. What the endpoint ACTUALLY returns is still measured per pass —
   * `assertDimension` — because a declared width is a plan, not a fact.
   */
  dimensions: number;
  /**
 ...
 * above.
 *
 * AND THE AUTOMATIC PROMOTION READS IT THE SAME WAY, which is the half that was
 * missing: `readOwnerChoices` in `clawai-cloud-defaults.ts` honours that mark
 * only where a PIN stands beside it, so a legacy box is promoted properly — the
 * pin written and the full rebuild asked for — instead of this reader moving it
 * to the cloud while the applier believed the owner had spoken and left the
 * index stamped for the other model. One mark, one meaning, on both sides.
 */
...
async function cloudEmbedder(): Promise<ResolvedEmbedder> {
  ...
  return {
    ...
    requestUrl: endpoint,
    dimensions: CLOUD_EMBEDDING_DIMENSIONS,
 ...
 *
 * THE FIELD SWITCH IS READ HERE, not only on the default path. `"off"` in
 * `CLAWAI_CLOUD_EMBEDDINGS_KEY` exists so a box already in a customer's hands
 * can be taken off the cloud embedder without an update, whatever its plan says
 * — and it used to be consulted only inside `probeCloudEmbeddings`, i.e. only
 * where nothing had been pinned. Since the automatic promotion writes a pin at
 * the first boot of every linked, paid box, that is precisely the population the
 * lever no longer reached: support switched it off and the owner's documents
 * kept going to the cloud. The switch now wins over a stored `cloud` too.
 *
 * It does NOT rewrite the pin. The lever is a support action and reversible; the
 * index it leaves behind reads `mismatched` and draws the card's existing "Run a
 * full reindex", exactly as any other embedder change does, and clearing the key
 * puts the box back where the owner left it.
 *
...
export async function resolveMemoryEmbedder(fallback?: EmbeddingSource): Promise<ResolvedEmbedder> {
  ...
  if (source !== "cloud") return localEmbedder();
  return (await cloudEmbeddingsSwitchedOff()) ? localEmbedder() : await cloudEmbedder();
}
...
export function embedEndpointAllowed(source: EmbeddingSource, baseUrl: string): boolean {
  ...
  // An empty endpoint on either side is not an address and must never compare
  // equal to one: `cloudEmbeddingsUrl()` answers "" on a box whose built-in
  // route did not pass its own trust boundary, and "" === "" would have turned
  // that refusal into a pass.
  const allowed = normalised(embeddingsBaseUrlOf(cloudEmbeddingsUrl()));
  if (!allowed || !normalised(baseUrl)) return false;
  return normalised(baseUrl) === allowed;
}
