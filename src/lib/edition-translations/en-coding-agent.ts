...
export const codingAgentEn: Record<string, string> = {
  ...
  "codingAgent.team.messagesTitle": "Messages ({n})",
  "codingAgent.team.inboxTitle": "Inbox for the lead ({n})",
  "codingAgent.team.messageToLead": "to the lead",
  ...
  "codingAgent.team.shape": "Sized for {n} side by side",
  "codingAgent.team.reviewMode.each": "a reviewer per task",
  "codingAgent.team.reviewMode.final": "one review of the merged result",
  "codingAgent.team.reviewMode.none": "rule check only",
  "codingAgent.team.leadOn": "Lead may change the plan",
  "codingAgent.team.addedByLead": "Added by the lead",
  "codingAgent.team.finalReview": "Final review",
  "codingAgent.team.metricsTitle": "What this team cost and how its shape fitted the goal",
  "codingAgent.team.metricTokens": "{n} tokens",
  "codingAgent.team.metricTime": "{time} in all",
  "codingAgent.team.metricFirstTry": "{n} of {total} accepted first try",
  "codingAgent.team.metricRejected": "{n} rejected at least once",
  "codingAgent.team.metricAdded": "{n} added by the lead",
  "codingAgent.team.metricRetired": "{n} retired",
  ...
  "codingAgent.teamDynamicLabel": "Let the team lead add or retire tasks while a team runs",
  "codingAgent.teamDynamicHint": "After each worker of a coding team finishes, the team's planner takes one more short, read-only look and may add up to three tasks or retire up to two that have not started. Each look is one more paid run. A team keeps the setting it started with.",
  "codingAgent.teamDynamicFailed": "Could not change the team lead setting.",
  ...
  "codingAgent.autoMergeHint": "When every check has passed, no review comment is left unanswered and nobody has asked for changes, ClawBox merges the pull request with a merge commit (a squash where the repository allows only that) and deletes its branch. While nothing is outstanding it also turns on GitHub's auto-merge, so the pull request merges the moment its required checks pass, even after ClawBox has stopped watching. Any base branch may be merged this way — beta, master, a feature branch — except main, which ClawBox never merges into however green the pull request is, and a pull request labelled hold is never merged. Off by default.",
  ...
  "codingAgent.stepAutoMergeOn": "Turned on GitHub's auto-merge for pull request #{number}: it merges the moment its required checks pass",
  "codingAgent.stepAutoMergeOff": "Turned off GitHub's auto-merge: {reason}",
  ...
  // Run history (TASK-1178): what the box keeps of finished runs — the card on
  // the settings page, and the Run history page with its two lists.
  "codingAgent.history.title": "Run history",
  "codingAgent.history.hint": "What the box keeps of finished runs, and where the older ones go. Live, paused and drafted runs are always kept.",
  "codingAgent.history.modeLabel": "Keep",
  "codingAgent.history.modeStandard": "Standard",
  "codingAgent.history.modeExtended": "Extended",
  "codingAgent.history.modeEverything": "Keep everything",
  "codingAgent.history.modeArchive": "Archive",
  "codingAgent.history.modeStandardHint": "The newest {count} runs. An older run is deleted with its evidence and inputs, and Claude Code deletes its own transcripts after 30 days.",
  "codingAgent.history.modeExtendedHint": "The newest {limit} runs, each with its evidence and inputs. Runs past the newest {count} are listed under Run history and open like any other run.",
  "codingAgent.history.modeEverythingHint": "Every run, with its evidence and inputs, and Claude Code is told to keep its transcripts too. Nothing is deleted unless you clear it.",
  "codingAgent.history.modeArchiveHint": "The newest {count} runs stay in the list. An older run, or one you clear, moves to the archive with its evidence, inputs and transcript, where you can read it and export it.",
  "codingAgent.history.limitLabel": "Runs to keep",
  "codingAgent.history.limitOption": "{count} runs",
  "codingAgent.history.saveFailed": "Could not change what the run history keeps.",
  "codingAgent.history.pendingDelete": "{count} older runs are past this setting and will be deleted when the next run starts. Switch back to keep them.",
  "codingAgent.history.pendingArchive": "{count} older runs will move to the archive when the next run starts.",
  "codingAgent.history.archiveTranscriptNote": "Claude Code still deletes its own transcripts after 30 days, so a run that stays in the list longer than that is archived without one. Choose Keep everything to keep them all.",
  "codingAgent.history.transcriptsKept": "Claude Code keeps its transcripts ({file}).",
  "codingAgent.history.transcriptsNotKept": "Claude Code may still delete old transcripts: {file} could not be updated. Choose the setting again to retry.",
  "codingAgent.history.transcriptsUnreadable": "{file} is not a settings file the box can read, so it was left as it is and Claude Code may still delete old transcripts.",
  "codingAgent.history.usage": "Run history uses {size} on this box.",
  "codingAgent.history.usageCounts": "{live} in the list · {older} older · {archived} archived",
  "codingAgent.history.usageFree": "{free} free",
  "codingAgent.history.usageUnknown": "Could not measure the run history.",
  "codingAgent.history.usageMeasuring": "Measuring the run history…",
  "codingAgent.history.lowDisk": "Less than {min} is free on this box. Until space is freed, a run leaving the newest {count} is deleted as in Standard, whatever this setting says. Nothing already kept is removed.",
  "codingAgent.history.open": "Open run history",
  "codingAgent.history.exportAll": "Export all history",
  "codingAgent.history.exportAllHint": "Downloads one .zip with every run record, evidence folder, input and Claude Code transcript the box still has, and the whole archive.",
  "codingAgent.history.clearArchive": "Clear archive",
  "codingAgent.history.clearArchiveConfirm": "Delete {count} archived runs?",
  "codingAgent.history.clearArchiveHint": "Deletes every archived run for good, with its evidence, inputs and transcript. The runs in the list and the older runs are not touched.",
  "codingAgent.history.clearArchiveFailed": "Could not clear the archive.",
  "codingAgent.history.clearArchiveDone": "Deleted {count} archived runs.",
  "codingAgent.history.tabOlder": "Older runs",
  "codingAgent.history.tabArchived": "Archived runs",
  "codingAgent.history.olderEmpty": "No older runs are kept. Choose Extended or Keep everything in Settings to keep more than the newest {count}.",
  "codingAgent.history.archivedEmpty": "The archive is empty. With Archive chosen in Settings, a run leaving the newest {count} moves here.",
  "codingAgent.history.loadFailed": "Could not read the run history.",
  "codingAgent.history.olderMeta": "{turns} turns · {files} files",
  "codingAgent.history.archivedAt": "archived {when}",
  "codingAgent.history.exportRun": "Export .zip",
  "codingAgent.history.readOnly": "Read-only",
  "codingAgent.history.notFound": "That run is not in the archive any more.",
  "codingAgent.history.detailStarted": "Started",
  "codingAgent.history.detailFinished": "Finished",
  "codingAgent.history.detailArchived": "Archived",
  "codingAgent.history.detailProject": "Folder",
  "codingAgent.history.detailSize": "Size",
  "codingAgent.history.detailTask": "Task",
  "codingAgent.history.detailSummary": "Summary",
  "codingAgent.history.detailEvidence": "Evidence",
  "codingAgent.history.detailInputs": "Files it was given",
  "codingAgent.history.detailFiles": "Files changed",
  "codingAgent.history.detailActivity": "Activity",
  "codingAgent.history.detailTranscript": "Transcript",
  "codingAgent.history.noEvidence": "This run saved no evidence.",
  "codingAgent.history.transcriptIncluded": "Claude Code's transcript is in the archive ({size}) and in the run's .zip.",
  "codingAgent.history.transcriptMissing": "Claude Code had already deleted this run's transcript when it was archived.",
  ...
  "codingAgent.stepTeamMessageToLead": "Team message to the lead: {text}",
  "codingAgent.stepTeamMessageToAssistant": "Team message to the assistant: {text}",
