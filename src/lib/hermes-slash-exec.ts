...
export interface HermesSlashResult {
  ...
  /**
   * The model the session is on AFTER the command, when Hermes said so.
   *
   * Not a guess and not a parse of the command: Hermes ANNOUNCES a model switch
   * on the socket that made it — `_commit_agent_switch` ends in
   * `_emit_session_info`, which writes a `session.info` event frame carrying
   * `{ model, provider, … }` to that session's own transport, which is this
   * socket because this module has just resumed the session on it.
   *
   * Empty when no such event arrived — a command that changed no model, or an
   * older gateway. Empty must therefore never be read as "unchanged"; the
   * caller's only use for it is a comparison it declines to make when this is
   * blank.
   */
  readonly sessionModel: string;
  /** The provider beside `sessionModel`, from the same event. */
...
interface RpcFrame {
  ...
  /** An EVENT frame carries no id and says what happened in `params`. */
  method?: unknown;
  params?: { type?: unknown; payload?: Record<string, unknown> };
}
 ...
 *
 * ClawBox's own sentence — one of the two in this module that are, and the
 * only one the harness could not have said: it has said nothing yet, by
 * definition. English like the rest of `chat-error-text.ts`. It exists because
 * the alternative — reporting the transport as unavailable — makes the caller
 * run the command a second time at the model while the first one is still
 * working.
 */
const STILL_RUNNING =
...
/** `sessionModel`/`sessionProvider`, present on every result shape. */
function announcedFields(
  announced: { model: string; provider: string } | null,
): { sessionModel: string; sessionProvider: string } {
  return {
    sessionModel: announced?.model ?? "",
    sessionProvider: announced?.provider ?? "",
  };
}

/**
 * What the owner is told when a command switched THIS CONVERSATION's model.
 *
 * ClawBox's own sentence, like `STILL_RUNNING`, and for a reason that belongs
 * to ClawBox rather than to Hermes: this chat carries a model on every turn —
 * the one its header names, which is the box's CONFIGURED model — and
 * `openDashboardTurn` re-points a resumed session at it whenever the two
 * differ. Hermes' own contract is that a plain or `--session` `/model` is
 * deliberately session-scoped and never written to config.yaml
 * (`resolve_persist_behavior`: "a plain `/model <name>` affects only the
 * current session"), so the two cannot both hold: Hermes answers with its own
 * success line and the next ordinary message puts the conversation back.
 *
 * Told nothing, the owner reads a switch that happened and was silently undone
 * — the false-success shape this codebase keeps producing. So it is said, from
 * a MEASUREMENT rather than a prediction: nothing here parses `/model`'s flags
 * or re-derives which forms persist. A switch that DID reach config.yaml
 * (`--global`) leaves the session on the same model the next turn will assert,
 * the comparison finds nothing, and this says nothing.
 */
export function describeSessionModelSwitch(
  /** What the session is on now, from Hermes' own `session.info` event. */
  sessionModel: string,
  /** The model THIS turn carried — which is what the next one will assert. */
  turnModel: string | undefined,
): string {
  const now = sessionModel.trim();
  const header = turnModel?.trim() ?? "";
  // No announcement, or no model on the turn at all (this chat sends none, so
  // nothing re-points the session): there is nothing to warn about.
  if (!now || !header || now === header) return "";
  return `This chat sends every message with the model named in its header (${header}), so your next message will put this conversation back on it. To change the model for good, pick it in the chat header — or run the command again with --global.`;
}

/** The command's output with that note under it, when there is one. */
export function withSessionModelNote(
  result: HermesSlashResult,
  turnModel: string | undefined,
): string {
  const note = describeSessionModelSwitch(result.sessionModel, turnModel);
  return note ? `${result.output}\n\n${note}` : result.output;
}
...
export async function runHermesSlashCommand(
  ...
  /** The newest `{ model, provider }` Hermes announced on this socket. */
  let announced: { model: string; provider: string } | null = null;

  socket.on("message", (raw: unknown) => {
    const frame = parseFrame(raw);
    if (!frame) return;
    if (typeof frame.id !== "number") {
      // An event, not a reply. `session.info` is the one this module reads: it
      // is how Hermes tells the client that a command changed what the session
      // runs, and it is the only honest answer to "what is it on now" that
      // costs no second RPC. Kept as the LATEST rather than the first, because
      // one command can emit more than one.
      if (frame.method === "event" && frame.params?.type === "session.info") {
        const payload = frame.params.payload ?? {};
        const model = typeof payload.model === "string" ? payload.model.trim() : "";
        const provider = typeof payload.provider === "string" ? payload.provider.trim() : "";
        if (model) announced = { model, provider };
      }
      return;
    }
  ...
  try {
    ...
    if (frame === TIMED_OUT) {
      return { output: STILL_RUNNING, sessionId, ...announcedFields(announced) };
    }
    ...
    if (frame.error) {
      ...
      return { output: errorText(frame) || NO_OUTPUT, sessionId, ...announcedFields(announced) };
    }
    ...
    return {
      output: readOutput(frame.result) || NO_OUTPUT,
      sessionId,
      ...announcedFields(announced),
    };
  } catch {
    return null;
  } finally {
