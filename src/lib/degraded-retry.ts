 ...
 * request (src/lib/hermes-model-options.ts — awaiting it was tried in #599 and
 * is wrong), so a poll can only return what a PREVIOUS poll's refresh
 * installed. The last poll therefore cannot recover anything: the deciding one
 * is the SECOND-TO-LAST. With six attempts the requests land at 0, 1, 3, 7, 15,
 * 23 and 31 s, so the dashboard has until ~23 s to come up — roughly twice the
 * measured boot window (`clawbox-setup` logs "Ready in 0ms" and starts serving
 * while `clawbox-hermes-dashboard` needs another 11-12 s). Five attempts read
 * as ~23 s and were really ~15 s, about three seconds of margin over a boot
 * that has been measured at twelve. Past the budget the surfaces say so
 * (`error` in useHermesModelOptions) rather than sitting on a spinner or
 * rendering the fallback as an answer.
 *
