...
export function commandsFromHermesCatalog(payload: unknown): SlashCommand[] {
  ...
  for (const pairRaw of pairs) {
    ...
    if (!id) continue;
    const disposition = asRecord(dispositions[id]);
    ...
    out.push({
      ...
      acceptsArgs: disposition ? disposition.argument_mode != null : false,
      source: "harness",
    });
  }
  return dedupe(out);
 ...
 * Whatever followed the token is kept — accepting a completion must never
 * delete text the owner had already typed after it.
 */
export function applySlashCommand(
  ...
  // The query IS the whole draft, so the command replaces all of it. Written as
  // a slice rather than assumed, so the two cannot drift if the query ever
  // becomes narrower than the draft again.
