...
export async function refreshCodingAgentToolsIfReadinessChanged(
  ...
  return refreshCodingAgentToolsIfChanged({ ready: before }, { ready: after }, options);
}

/**
 */
/**
 * The facts this box's tool list is built from — the ones the device can move
 ...
 *
 *
 * @param before the facts BEFORE this request's write
 * @param after  the facts after it — the same status the panel is shown
 * @param options see `CodingAgentRefreshOptions`
 */
