...
it("allows only a live run's own loopback listener in its project and process group", async () => {
  const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "preview-owner-"));
  const child = spawn(process.execPath, ["-e", "const s=require('http').createServer((q,r)=>r.end('preview'));s.listen(0,'127.0.0.1',()=>console.log(s.address().port));"], { cwd, detached: true, stdio: ["ignore", "pipe", "pipe"] });
  try {
