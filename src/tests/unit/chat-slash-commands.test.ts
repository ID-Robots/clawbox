...
describe("the Hermes command source (commands.catalog)", () => {
  ...
  it("inserts a row the `commands` map omits by its own PUBLISHED hint", () => {
    // The catalogue's two halves can disagree: `usage` needs nothing but the
    // `pairs` row, `argument_mode` lives only in `commands`. A row present in
    // one and absent from the other used to render as `/undo [n]` — visibly
    // promising an argument — and go in BARE with the caret at the end, so the
    // very next Enter sent it with nothing attached. That is the `/btw`
    // failure `acceptsArgs` exists to prevent.
    const partial = {
      ...HERMES_CATALOG,
      commands: { "/new": { argument_mode: null, desktop: null } },
    };
    const byId = new Map(commandsFromHermesCatalog(partial).map((c) => [c.id, c]));
    expect(byId.get("/undo")?.usage).toBe("/undo [n]");
    expect(byId.get("/undo")?.acceptsArgs).toBe(true);
    // …and a row with no hint and no disposition still says no, because
    // nothing published anything about arguments for it.
    expect(byId.get("/status")?.acceptsArgs).toBe(false);
    // A row the map DOES describe is still the map's answer, hint or not.
    expect(byId.get("/new")?.acceptsArgs).toBe(false);
  });

  it("finds a disposition keyed WITHOUT the leading slash", () => {
    // `toCommandId` deliberately accepts `name` and `/name`; looking the
    // disposition up only under the slashed key made a catalogue that keys the
    // map the other way degrade silently to "offer everything, no arguments
    // anywhere" — the `desktop` filter and the argument flag both no-ops.
    const bare = {
      ...HERMES_CATALOG,
      commands: {
        new: { argument_mode: null, desktop: null },
    ...
    const rows = commandsFromHermesCatalog(bare);
    expect(rows.map((c) => c.id)).toEqual(["/new", "/status", "/undo"]);
    expect(rows.find((c) => c.id === "/undo")?.acceptsArgs).toBe(true);
  });
