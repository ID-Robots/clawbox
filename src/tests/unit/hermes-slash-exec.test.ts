...
import {
  describeSessionModelSwitch,
  runHermesSlashCommand,
  withSessionModelNote,
} from "@/lib/hermes-slash-exec";
...
describe("runHermesSlashCommand", () => {
  it("resumes the stored session, then runs the command through slash.exec", async () => {
    ...
    expect(await call).toEqual({
      ...
      // Nothing announced a model, so nothing is claimed about one. Blank is
      // "Hermes did not say", never "unchanged".
      sessionModel: "",
      sessionProvider: "",
    });
  ...
  it("creates a session when there is none to resume, carrying the pairing", async () => {
    ...
    expect(await call).toEqual({ output: "ok", sessionId: "20260917_120000_ffee11", sessionModel: "", sessionProvider: "" });
  });
  ...
  it("follows Hermes' own instruction to re-route a skill command to command.dispatch", async () => {
    ...
    expect(await call).toEqual({ output: "drawn", sessionId: "20260917_090000_aabbcc", sessionModel: "", sessionProvider: "" });
  });
  ...
  it("hands back a refusal as OUTPUT — a command that ran and failed is an answer", async () => {
    ...
    expect(await call).toEqual({ output: "unknown command: /frobnicate", sessionId: "20260917_090000_aabbcc", sessionModel: "", sessionProvider: "" });
  });
  ...
  it("reads a command.dispatch result that carries no `output` at all", async () => {
    ...
    expect(await call).toEqual({ output: "Backed up 2 turns.", sessionId: "20260917_090000_aabbcc", sessionModel: "", sessionProvider: "" });
  });
  ...
  it("never answers null once the gateway has run the command, whatever it said", async () => {
    ...
    expect(await call).toEqual({ output: "(no output)", sessionId: "20260917_090000_aabbcc", sessionModel: "", sessionProvider: "" });
  });
  ...
  it("reports the model Hermes ANNOUNCED after the command, from its own event", async () => {
    // `_commit_agent_switch` ends in `_emit_session_info`, which writes a
    // `session.info` event frame to the session's own transport — this socket,
    // because this module has just resumed the session on it. The frame
    // carries no `id`, so it used to be dropped on the floor and the caller had
    // no way to learn that `/model` had changed anything.
    const call = runHermesSlashCommand({ command: "/model gpt-5.6-sol", sessionId: "20260917_090000_aabbcc" });
    const socket = await latest();
    socket.open();
    await untilSent(socket, 1);
    socket.deliver({ jsonrpc: "2.0", id: 1, result: { session_id: "rt1", stored_session_id: "20260917_090000_aabbcc" } });
    ...
    socket.deliver({
      jsonrpc: "2.0",
      method: "event",
      params: { type: "session.info", session_id: "rt1", payload: { model: "gpt-5.6-sol", provider: "openai" } },
    });
    socket.deliver({ jsonrpc: "2.0", id: 2, result: { output: "Switched to gpt-5.6-sol" } });
    expect(await call).toEqual({
      output: "Switched to gpt-5.6-sol",
      sessionId: "20260917_090000_aabbcc",
      sessionModel: "gpt-5.6-sol",
      sessionProvider: "openai",
    });
  });

  it("keeps the LAST announcement when a command emits more than one", async () => {
    const call = runHermesSlashCommand({ command: "/model a", sessionId: "20260917_090000_aabbcc" });
    const socket = await latest();
    socket.open();
    await untilSent(socket, 1);
    socket.deliver({ jsonrpc: "2.0", id: 1, result: { session_id: "rt1", stored_session_id: "20260917_090000_aabbcc" } });
    ...
    const info = (model: string) => ({
      jsonrpc: "2.0",
      method: "event",
      params: { type: "session.info", session_id: "rt1", payload: { model, provider: "openai" } },
    });
    socket.deliver(info("first"));
    socket.deliver(info("second"));
    // An event of another kind changes nothing, and neither does one with no
    // model on it — "Hermes did not say" must never overwrite what it did say.
    socket.deliver({ jsonrpc: "2.0", method: "event", params: { type: "compacted", session_id: "rt1", payload: {} } });
    socket.deliver(info(""));
    socket.deliver({ jsonrpc: "2.0", id: 2, result: { output: "done" } });
    expect((await call)?.sessionModel).toBe("second");
  });
});

describe("what the owner is told about a session-only model switch", () => {
  it("names the model the NEXT message will assert, and both ways to keep this one", () => {
    const note = describeSessionModelSwitch("gpt-5.6-sol", "deepseek-v4-flash");
    expect(note).toContain("deepseek-v4-flash");
    expect(note).toContain("--global");
    // Never the other way round: the sentence is about what is coming back,
    // not about what the owner just picked.
    expect(note).not.toContain("gpt-5.6-sol");
  });

  it("says nothing when the two agree, when either is missing, or when Hermes was silent", () => {
    // `--global` wrote config.yaml, so the header and the session match. This
    // is measured, not predicted: no flag is parsed anywhere.
    expect(describeSessionModelSwitch("deepseek-v4-flash", "deepseek-v4-flash")).toBe("");
    // A surface that sends no model re-points nothing, so there is nothing to
    // warn about — the full-screen chat is exactly that case.
    expect(describeSessionModelSwitch("gpt-5.6-sol", undefined)).toBe("");
    expect(describeSessionModelSwitch("gpt-5.6-sol", "")).toBe("");
    // No announcement is "Hermes did not say", never "it changed".
    expect(describeSessionModelSwitch("", "deepseek-v4-flash")).toBe("");
  });

  it("puts the note under the harness's own output and never inside it", () => {
    const ran = {
      output: "Switched to gpt-5.6-sol",
      sessionId: "20260917_090000_aabbcc",
      sessionModel: "gpt-5.6-sol",
    ...
    expect(withSessionModelNote(ran, "deepseek-v4-flash")).toBe(
      `Switched to gpt-5.6-sol\n\n${describeSessionModelSwitch("gpt-5.6-sol", "deepseek-v4-flash")}`,
    );
    // Nothing to add, nothing added — not even a blank line.
    expect(withSessionModelNote({ ...ran, sessionModel: "deepseek-v4-flash" }, "deepseek-v4-flash")).toBe(
      "Switched to gpt-5.6-sol",
    );
  });
});
