...
describe("plugin-repair — which row a failure belongs to", () => {
  ...
  it("puts the DeepSeek plugin on the ClawBox AI row and Codex on OpenAI", async () => {
    ...
    const repairs = {
      deepseek: { id: "@openclaw/deepseek-provider", stage: "install" as const, reason: "r", atMs: 1, disabled: true },
      codex: { id: "codex", stage: "consent" as const, reason: "r", atMs: 1, disabled: true },
    };
