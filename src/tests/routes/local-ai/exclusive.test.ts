 ...
 * enable has to precede the restore write here just as it does at the other
 * two sites.
 */
...
vi.mock("@/lib/openclaw-session-store", () => ({
  listAgentIds: vi.fn(async () => []),
  sessionStorePath: vi.fn((agentId: string) => `/nonexistent/${agentId}/sessions.json`),
  sweepSessionEntries: vi.fn(),
}));
