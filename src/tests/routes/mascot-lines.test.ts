...
describe("POST /setup-api/mascot-lines/regenerate", () => {
  ...
  it("tells the three refusals apart instead of collapsing them into one string", async () => {
    // All three used to answer with the same "unavailable right now" text, so
    // a Settings button could not tell "your chat is using the model, try
    // again in a minute" apart from a real fault.
    const messages = new Set<string>();
    for (const reason of ["busy", "low-memory", "malformed"] as const) {
      forceRegenerate.mockResolvedValue({ phrases: null, reason, locale: "bg" });
      const body = await (await POST(request("?locale=bg"))).json();
      expect(body.ok).toBe(false);
      expect(body.meta).toEqual({ source: "pack", reason, locale: "bg" });
  ...
  it("passes an unshipped locale through as null, like GET does", async () => {
    forceRegenerate.mockResolvedValue({ phrases: null, reason: "busy", locale: "en" });
