...
vi.mock("@/lib/hermes-dashboard-control", () => ({
  ...
  hermesDashboardMainPid: vi.fn(async () => ({ read: true, pid: 4242 })),
  hermesDashboardServing: vi.fn(async () => true),
  hermesDashboardUnitState: vi.fn(async () => "running"),
}));
...
describe("POST /setup-api/hermes/plugins/reload", () => {
  ...
  it("refuses a POST fired from ANOTHER SITE'S page, session cookie and all", async () => {
    // The owner's browser attaches the session cookie to a POST any page on the
    // web fires at the box, and this one drops their chat window — a site they
    // merely visited could restart the assistant as often as it liked.
    const cross = new Request("http://localhost/setup-api/hermes/plugins/reload", {
      method: "POST",
  ...
  it("still admits the header-less MCP bearer, which is the intended caller", async () => {
    // The MCP server sends neither `Origin` nor `Sec-Fetch-Site`; its credential
    // is what the gate above decides on. A same-origin check that refused a
    // header-less caller would have taken the tool away from the agent.
    const response = await post(req("agent"));
    expect(response.status).toBe(200);
    expect(bounceMock).toHaveBeenCalledTimes(1);
  });

  it("admits the owner's own page, which sends an Origin of its own", async () => {
    const own = new Request("http://localhost/setup-api/hermes/plugins/reload", {
      method: "POST",
      headers: { Cookie: session.cookie, Origin: "http://localhost", Host: "localhost" },
    });
