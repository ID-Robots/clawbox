/**
 * GET/POST /setup-api/whats-new — the "What's new in 4.0" card (TASK-1059).
 *
 * What is pinned here:
 *  - the card is for a box RUNNING 4.x, read from the checkout's package.json;
...
describe("GET /setup-api/whats-new: when the card shows", () => {
  it("shows on a box running 4.0.0 that has not dismissed it", async () => {
  ...
  it.each(["4.0.3", "4.1.0", "v4.0.0"])("shows on %s, which is on the 4.x line", async (version) => {
    writeVersion(version);
    expect((await state()).show).toBe(true);
  });

  it.each(["3.9.0", "3.9.12", "5.0.0", "not-a-version"])("does not show on %s", async (version) => {
    writeVersion(version);
  ...
  it("falls back to the build-time version when package.json is unreadable", async () => {
    writeVersion(null);
    process.env.NEXT_PUBLIC_APP_VERSION = "v4.0.0-12-gabc1234";
    const body = await state();
    expect(body.version).toBe("v4.0.0-12-gabc1234");
  ...
  it("a dismissal of some other release does not hide this one", async () => {
    writeConfig({ whats_new_dismissed: "3.9" });
    expect((await state()).show).toBe(true);
  });
});
...
describe("POST /setup-api/whats-new: dismissal", () => {
  it("records the dismissal in the box's config store, for every browser", async () => {
    ...
    expect(readConfig()).toEqual({ keep: "me", whats_new_dismissed: "4.0" });
  ...
  it("refuses a body far larger than a dismissal", async () => {
    const res = await post({ release: "4.0", padding: "x".repeat(4096) });
    expect(res.status).toBe(413);
    expect(readConfig()).toEqual({});
  });

  it("refuses a dismissal fired from another site's page", async () => {
    const res = await post({ release: "4.0" }, { origin: "https://evil.example" });
