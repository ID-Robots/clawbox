...
- **`TerminalTabs.tsx`** — the Terminal window: several shells as tabs, every tab's `TerminalApp` kept mounted (its PTY survives a switch; the panel is `hidden`), the first tab carrying the window's `initialCommand` and named after it, Ctrl+Shift+T / Ctrl+Shift+W / Ctrl+PageUp/PageDown handed up from the terminal (`onTabAction`), closing the last tab opening a fresh one. `TerminalApp` itself gained `active` (focus + refit when the tab comes to the front) and a right-click menu (Copy — over plain HTTP through the execCommand fallback —, Paste when the origin can read the clipboard and named as Ctrl+Shift+V when it cannot, Select all, Clear; `rightClickSelectsWord` so a right click on a word selects it). The live preview still embeds the single `TerminalApp`. Terminal strings are `terminal.*` keys in the desktop translations.
- **`CodingRunLivePreview.tsx`** — the floating live terminal of a coding run: a small `TerminalApp` tailing the run's transcript (`scripts/coding-run-preview`) popped over the desktop from the chat run card's Live view button (`dispatchCodingLivePreview`, `CODING_LIVE_PREVIEW_EVENT`); a click on its bar folds it to the bar and back, the X closes it, and "Open in Terminal" hands the same command to a full Terminal window. `page.tsx` owns exactly one. The desktop's coding finish cards hide themselves after `CODING_NOTICE_MS` and their Open button lands on the run's page.
- **`ChromeWindow.tsx`** — the desktop window. While the chat is docked (`rightInset > 0`) every window fills the pane beside it with `DOCK_FILL_MARGIN` on all sides — no dragging, resizing, snapping or maximize meanwhile — and gets its own geometry back the moment the chat undocks.
